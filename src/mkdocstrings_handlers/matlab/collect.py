...
from mkdocstrings_handlers.matlab.models import Namespace, Class
...
class PathCollection(ModulesCollection):
    ...
    def __init__(
        ...
        self._database: dict[Path, PathMixin] = {}

        for path in matlab_path:
            self.addpath(Path(path), to_end=True)

    def resolve(
        self, name: str, local_namespaces: list[Path] | None = None
    ) -> PathMixin | None:
        """
        ...
        if local_namespaces is not None:
            for str_path in local_namespaces:
                path = Path(str_path)
                if (
                    path in self._local_namespaces
                    and name in self._local_namespaces[path]
                ):
    ...
    def _add_package_to_local_namespace(
        self, path: Path, package: Namespace, to_end: bool = False
    ):
        """
    ...
    def rm_path(self, path: str | Path, recursive: bool = False):
        ...
        if recursive:
            for subdir in [
                item for item in self._search_path if _is_subdirectory(path, item)
            ]:
    ...
    def resolve_dependencies(self) -> None:
        ...
        for node in nodes:
            if not isinstance(node, PathMixin):
                continue
    ...
    def _resolve_node(self, node):
        ...
        packages = [
            self.resolve(pkg, local_namespaces=[node.path.parent])
            for pkg in node._imports
        ]

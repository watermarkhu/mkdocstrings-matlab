??? code "Source files"

    === ":material-file-tree: tree"

        ```tree
        +module
            +submodule
                Contents.m
                subfunction.m
            aClass.m
            do_something.m
            readme.m
        ```

    === ":material-file-code: `readme.md`"

        ```matlab
        --8<-- "docs/src/snippets/+module/readme.md"
        ```

    === ":material-file-code: `do_something.m`"

        ```matlab
        --8<-- "docs/src/snippets/+module/do_something.m"
        ```

    === ":material-file-code: `aClass.m`"

        ```matlab
        --8<-- "docs/src/snippets/+module/aClass.m"
        ```

    === ":material-file-code: `subfunction.m`"

        ```matlab
        --8<-- "docs/src/snippets/+module/+submodule/subfunction.m"
        ```

    === ":material-file-code: `Contents.m`"

        ```matlab
        --8<-- "docs/src/snippets/+module/+submodule/Contents.m"
        ```
